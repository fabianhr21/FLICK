...
START, END, DT = 150000, 240000, 10000
...
mesh.xyz = pyQvarsi.truncate(mesh.xyz,6)
...
for instant in listOfInstants[1:]:
	...
	for var in VARLIST:
		avgField[var] += pyQvarsi.stats.addS1(avgField[var],field[var],w=dt/time)

# Compute the derivative of AVVEL to compute the RETAU
# from the averaged field data
gradv = mesh.gradient(avgField['AVVEL'])
tw    = mu*np.abs(gradv[:,1]) # du/dy
avgField['RETAU'] = np.sqrt(tw/rho)/mu


## Average in XZ plane and results in Y direction
avgYField,_ = pyQvarsi.postproc.directionAvg(avgField,direction='y')
...
avgYField['COUNT'] = np.ones((len(avgYField,)),np.double)
...
avgYFieldG = avgYField.reduce(root=0,op='sum')
...
if pyQvarsi.Communicator.serial(): # Returns true if rank == 0 or MPI_SIZE == 1
	...
	avgYFieldG /= avgYFieldG['COUNT']
	print('Reduction step done!',flush=True)

	# Sort according avgYFieldG
	# here we use the whole xyz vector since avgYFieldG
	# only contains y directions
	avgYFieldG.sort(array='y')

	# Recover arrays
	y_pos = avgYFieldG.y 
	avvel = avgYFieldG['AVVEL']
	avve2 = avgYFieldG['AVVE2']
	retau = avgYFieldG['RETAU']
	...
	# Load data from DNS
	dns = np.loadtxt('./Re950_DNS.dat', delimiter=',', skiprows=27)
