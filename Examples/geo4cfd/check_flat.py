...
_ANSA_SCRIPTS_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "flick_urban", "geo4cfd", "ansa")
...
def GroundCreate(x_min,x_max,y_min,y_max,z_min,z_max,avg_height):
	deck = constants.NASTRAN
	 ...
	 ## Create Morph box
	min_coords = [x_min-avg_height,y_min-avg_height,z_min]
	max_coords = [x_max+avg_height ,y_max+avg_height ,z_min]
...
def main():
    ...
    print(f"Reading STL file: {file_path + input_file + '.stl'}")
    ...
    if os.path.exists(target_path + "domain_dimensions.txt"):
        with open(target_path + "domain_dimensions.txt", "r") as domain_file:
            lines = domain_file.readlines()
            for line in lines:
                if line.startswith("avg_h"):
                    avg_height = float(line.split('=')[1].strip())
                    print("Average building height from file:", avg_height)
                    break
    ...
    if os.path.exists(target_path + "domain_dimensions.txt"):
        with open(target_path + "domain_dimensions.txt", "a") as domain_file:
        ...
        # Create domain_dimensions.txt and write dimensions
